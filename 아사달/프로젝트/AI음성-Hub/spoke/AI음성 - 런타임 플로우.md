...
> 8-18 판은 완화 임계값(200ms/500ms/RMS120)과 P1-1(barge-in 실패 시 누적 유지) 기준이었다. 이후 사용자 결정으로 **게이트가 원본(v2.6.7) 기준으로 되돌아갔다** — 8-19 임계값 원본 복귀(1000/3/300/600) 후 min_utterance·barge_in 만 **1200** 으로 상향, 8-20 **barge-in 정산(관문 미달 시 폐기·리셋) 복귀**(P1-1 되돌림)와 **cedar 음성 복원**, 그리고 **침묵 종료만 60ms → 400ms 재상향**(어절 사이 쉼 조각화 방지). 드레인 4배속(P1-2)과 force_flush 관문 우회는 유지된다. 실발화 기준은 같은 날 300 → 600/900 → 400/600 → 600 → **400/600** 으로 조정을 반복했다. 패스스루 해제 기준도 실발화 부재(RMS<400)로 분리했고(배경음 고착 결함), **barge-in 트리거를 소리 → 실발화 기준으로 변경**했다(잡음이 시계를 채우는 조기 발동 결함 — force_flush 는 관문 적용 정산으로 전환). 이 판은 `fix/v4-session-generation-guard-dev` `0dfbbbb` 코드 기준이다. 위치 표기: `router:` = voice_chat_v4_router.py, `sw:` = server_webrtc.py, `at:` = v4/audio_tracks.py, `orch:` = v4/tool_call_orchestrator.py.
...
flowchart TD
    ...
    B11 -->|"실발화(RMS>400) 1.2초 누적<br/>barge-in · at:600"| B12["<b>즉시 정산</b><br/>통과: 방출 + 실시간 패스스루<br/>미달: 폐기·리셋 (잡음은 시계를 못 채움)"]
    B11 -->|"침묵 400ms 20프레임<br/>발화 종료"| B13["<b>침묵 종료 정산</b> · at:494<br/>통과면 방출,<br/>미달이면 통째 폐기 (OpenAI 미도달)"]
    B11 -->|"소리 30초 누적<br/>force_flush"| B14["<b>강제 정산 (관문 적용)</b> · at:620<br/>침묵 없는 잡음 환경의 버퍼 무한 누적 방지.<br/>통과분 방출 · 잡음은 폐기 (우회 아님)"]
...
| 3관문 (원본 복귀 + 8-20 조정) | at:482  | ① 발화 길이 **1200ms** ② 실발화(RMS>**400**) 누적 **600ms** ③ 실발화 비율 **30%**. `_gate_passes()` 가 부작용 없는 단일 출처다. 8-19 원본(1000/300/600) 복귀 후 min_utterance 만 1200 상향 — 상세는 아래 [[#게이트 파라미터 (2026-08-20 기준)]] 표 참고. 7필드 전부 세션 생성 요청으로 주입 가능(router:72)                         |
| 발화 종료 판정              | at:410  | 연속 침묵 **20프레임(400ms)** 이면 발화 종료 → 정산. 원본값 3(60ms)은 어절 사이 쉼마다 발화를 쪼개 통째 소실을 만들어(8-14 실측) 8-20 에 400ms 로 재상향했다                                                                                                                                                      |
| barge-in              | at:600  | **실발화(RMS>400) 누적 1.2초** 도달 시 침묵을 기다리지 않고 **즉시 정산** — 그 시점에 3관문 전체를 검사해 통과면 방출 + 실시간 패스스루, 미달이면 폐기·리셋. 트리거가 소리(voice) 기준이던 시절엔 잡음이 시계를 채워 0.6초만 말해도 발동했다(8-20 실사용 확인, 실발화 기준으로 변경). 패스스루는 **실발화 부재(RMS<400) 400ms** 에 해제 — 무음(50) 기준이면 약한 배경음만으로 고착되던 결함도 같은 날 수정 |
| force_flush           | at:620  | 소리 누적 30초 시 **관문을 적용해 정산** — 통과분은 방출, 미달(잡음)은 폐기. barge-in 이 실발화 기준이 되면서 잡음-only 버퍼는 침묵 없는 환경에서 정산 기회가 없어 이 주기가 유일한 수거 경로다. 구 동작(관문 우회 방출)은 잡음 30초치를 통째로 쏟는 사고라 8-20 폐기                                                                                           |
...
`NoiseFilteredAudioProxyTrackV4` 생성자 기본값. 7개 필터 필드는 전부 세션 생성 요청으로 챗봇·환경별 주입이 가능하다(안 보낸 값만 기본값 사용).
...
| `silence_frames_to_end` | **20** (400ms) | 발화 종료 판정 침묵 | 연속 침묵(RMS≤50) 20프레임이면 발화 종료 → 정산. 원본 60ms 는 쉼마다 발화를 쪼개 8-20 재상향. 패스스루 해제도 같은 프레임 수를 쓰되 음량 기준은 **speech_rms(400) 미만** — 무음 기준이면 약한 배경음으로 고착된다(8-20 수정) |
| `speech_rms_threshold` | **400** | 진짜 발화 음량 기준 | RMS 400(≈ -38dBFS, 풀스케일 1.2%) 초과 프레임만 "진짜 말"로 집계(speech_duration). 순수 음량 기준이라 큰 소음도 집계되고 조용한 말은 누락된다 — 내용 판단이 아니다. 8-20 에 300 → 600 → 400 → 600 → **400** 으로 여러 번 조정했다. OpenAI VAD 하한(120)보다 여전히 보수적이라 모바일(AGC 진폭 저하) 취약 |
| `min_speech_duration_ms` | **600** | 진짜 발화 최소 누적 | 진짜 말 프레임의 누적이 600ms 이상이어야 통과(원본값 — 900 상향은 dev 로그 실측으로 당일 되돌림). 비율이 아니라 **절대 시간**이며, 연속일 필요 없이 발화 안에서 합산한다. barge-in 경로는 트리거 자체가 실발화 1200ms 라 이 관문을 자동 충족하고, 침묵 종료·force_flush 정산에서 실질 관문으로 작동한다 |
| `speech_ratio_threshold` | **0.30** | 진짜 발화 비율 | speech÷voice ≥ 30%. 배경소음이 길게 깔린 발화(비율 4~18% 실측)를 차단하는 시간 비율 |
| `barge_in_ms` | **1200** | 끼어들기 판정 시간 | **실발화(RMS>400) 누적** 1.2초 도달 시 즉시 정산 — 3관문 통과면 방출 + 패스스루, 미달이면 폐기·리셋. 소리 기준이던 시절의 잡음 조기 발동을 8-20 수정 |
| `force_flush_ms` | **30000** | 정산 주기 상한 | 소리 누적 30초 시 관문을 적용해 정산(통과 방출·미달 폐기). 침묵 없는 잡음 환경에서 버퍼 무한 누적을 막는 유일한 수거 경로 (8-20 우회 방출 → 관문 적용으로 변경) |
| `_DRAIN_SPEEDUP` | **4** | 드레인 배속 | 통과 발화 백로그를 4배속 페이싱으로 방출 (주입 불가, 클래스 상수) |
| `target/max_buffer_ms` | **80 / 960** | 업링크 지터버퍼 | 프레임 도착 출렁임 흡수 목표/상한. 상한은 8-21 에 320 → **960** (모바일 실측 지터 633~772ms). v3 사본은 여전히 320 이다 |

판정 흐름 한 줄 요약.

```
프레임(20ms)마다:  RMS > 50 → voice 누적,  RMS > 400 → speech 도 누적
정산 시점:  실발화 1.2초 누적(barge-in) / 침묵 400ms(발화 종료) / 소리 30초(force_flush)
3관문 AND: voice ≥ 1200ms  ∧  speech ≥ 600ms  ∧  speech/voice ≥ 30%
통과 → 드레인 4배속 방출  ·  미달 → 통째 폐기 (OpenAI 미도달)
```

**대기시간 미사용은 게이트를 아예 안 태운다** (2026-08-21 `c5e754b`). 기획서의 사용/미사용 라디오는 임계값 조정이 아니라 **트랙 교체**로 구현했다. 세션 생성 요청의 `wait_time_enabled=False` 면 라우터가 게이트 대신 `UplinkAudioProxyTrackV4` 를 주입하고 필터 임계값은 아예 넘기지 않는다.

```
wait_time_enabled=True   NoiseFilteredAudioProxyTrackV4  지터버퍼 + 3관문 + barge-in + 드레인
wait_time_enabled=False  UplinkAudioProxyTrackV4         지터버퍼만 (관문 없음)
```

`server_webrtc` 의 브릿지는 `audio_track_cls is not None` 일 때만 트랙을 감싸므로 원래 `None` 을 넘겨도 됐지만, 그러면 지터버퍼까지 사라진다. **상한 960ms 는 모바일 실측 지터 대응이라 잃으면 안 되는 값이다.**

미사용이어도 끼어들기가 사라지지는 않는다. `semantic_vad` + `interrupt_response=True` 가 그대로 맡고, 게이트의 소음 차단 이득은 어차피 0 으로 실측돼 있다 — 미사용은 "품질 포기" 가 아니라 **OpenAI 기본 동작으로 복귀**다.

대기시간을 낮출 때 3필드를 함께 보내야 하는 이유는 [[음성 선택과 대기시간 옵션]] 참고.
...
- [[AI음성]] — 프로젝트 진행 상황
- [[AI음성 - 파일·함수 레퍼런스]] — 어느 파일을 고쳐야 하는지 (v4 심층)
- [[AI음성 - 폴더·파일 역할 지도]] — ai_voice 전체 폴더·파일이 무슨 역할을 담당하는지
...
- [[음성 선택과 대기시간 옵션]] — 게이트 파라미터를 기획서 대기시간 옵션에 잇는 규칙
