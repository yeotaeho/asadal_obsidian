...
2026-08-31에 축을 둘 넓혔다 — 요청을 **보내는** 쪽(httpx)과, 애초에 **무엇이 설치되는지**(uv).
...
4. [[httpx — 요청을 보내는 쪽]] — FastAPI와 방향이 반대다. async 이득의 원리, tenacity 재시도
5. [[uv — 의존성과 환경 재현]] — `pyproject.toml`(의도)과 `uv.lock`(사실), 개발 PC와 서버를 같게 만들기
...
> 그 "붙잡는 냄비"의 가장 흔한 정체가 **동기 HTTP 호출**이다 → [[httpx — 요청을 보내는 쪽]]
